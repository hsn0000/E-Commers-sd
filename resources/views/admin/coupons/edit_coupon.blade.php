...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"> <a href="{{url('/admin/dashboard')}}" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> {{__('backend.home')}}</a> <a href="{{$module->permalink}}">{{__('backend.coupons')}}</a>
     <a href="#" class="current">{{__('backend.edit_coupon')}} </a> </div>
        ...
        <div class="widget-box">
          <div class="responsif-costume">
            ...
            <div class="widget-content nopadding">
              <form action="{{ $module->permalink.'/update' }}" class="form-horizontal" id="form-table" method="post" autocomplete="off">
                {{csrf_field()}}
                  <div class="control-group">
                    <label class="control-label required">{{__('backend.coupon_code')}}</label>
                    <div class="controls">
                      <input type="text" name="coupon_code" id="coupon_code" class="coupon_code" value="{{old('coupon_code') ?: $couponDetails->coupon_code}}" style=" width: 275px; @error('coupon_code') border-style: solid; border-color: orangered; @enderror ">
                  ...
                  <div class="control-group">
                    ...
                    <div class="controls">
                      <select name="amount_type" id="amount_type" style=" width: 289px; @error('amount_type') border-style: solid; border-color: orangered; @enderror ">
                          <option value="null" @if($couponDetails->amount_type == "") selected="selected" @endif > select data </option>
                          <option value="Persentage" @if($couponDetails->amount_type == "Persentage") selected="selected" @endif >{{__('backend.persentage')}}</option>
                          <option value="Fixed"  @if($couponDetails->amount_type == "Fixed") selected="selected" @endif >{{__('backend.fixed')}}</option>
                      </select> 
                  ...
                  <div class="control-group">
                    ...
                    <div class="controls">
                      <input class="" type="text" name="amount" id="amount" min="" value=" @if(old('amount')) {{old('amount')}} @elseif($couponDetails->amount_type == 'Persentage'){{ $couponDetails->amount.' %'}} @elseif($couponDetails->amount_type == 'Fixed') {{'Rp.'.$couponDetails->amount}} @endif " style=" width: 275px; @error('amount') border-style: solid; border-color: orangered; @enderror ">
                  ...
                  <div class="control-group">
                    ...
                    <div class="controls">
                    <input type="text" name="expiry_date" id="expiry_date" value="{{old('expiry_date') ?: $couponDetails->expiry_date}}" autocomplete="off" style=" width: 275px; @error('expiry_date') border-style: solid; border-color: orangered; @enderror "> 
                  ...
                  <div class="control-group">
                    <div class="controls">
                        <label class="control-input-content"> Enable 
                            <div class="switch">
                                <input type="checkbox" name="status" id="status" value="1" class=" toggle-switch-primary" @if($couponDetails->status == 1) checked @endif>
